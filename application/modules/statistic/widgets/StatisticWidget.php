...
class StatisticWidget extends Widget
{
    ...
    public function run()
    {
        ...
        $ordersNow = Order::find()->leftJoin('order_date', 'orders.id=order_date.order_id')->where(['order_date.date' => \Yii::$app->request->get('deliveryDate') ? \Yii::$app->request->get('deliveryDate') : date('d.m.Y')])->andWhere(['is_close' => false]);
