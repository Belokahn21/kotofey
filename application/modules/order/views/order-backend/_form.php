...
<div class="tab-content" id="backendFormsContent">
    ...
    <div class="tab-pane fade" id="nav-history-edit" role="tabpanel" aria-labelledby="nav-history-edit">
        ...
        <?php if ($mail = OrderMailHistory::findByOrderId($model->id)): ?>
            <?= Html::a('Отправлено письмо: #' . $mail->id, Url::to(['order-mail-history-backend/update', 'id' => $mail->id])); ?>/<?= Html::a('Удалить письмо #' . $mail->id, Url::to(['order-mail-history-backend/delete', 'id' => $mail->id])); ?>
        <?php endif; ?>
    </div>
