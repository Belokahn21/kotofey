...
$this->title = Title::showTitle("Список заказов"); ?>
    <section class="new-order-block">
        ...
        <div class="left-col">
            <h1 class="title">Новый заказ</h1> <span class="order-summ">Сумма заказа: <span class="order-summ-count"><?= rand(); ?></span><?= (new Currency())->show(); ?></span>
            ...
            <?= $form->field($model, 'user_id')->widget('\app\widgets\SelectCustomer')->label(false); ?>
        </div>
        <div class="right-col">
...
<?= GridView::widget([
    ...
    'columns' => [
        ...
        [
            'attribute' => 'user',
            'value' => function ($model) {
                return User::findOne($model->user)->email;
            }
        ],
        [
            'attribute' => 'cash',
