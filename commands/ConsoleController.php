	...
	public function actionPrice()
	{
		...
		foreach ($products->all() as $product) {
			...
			if ($percent = $sale[$product_weight->value]) {
				if (in_array($product->id, $show_product)) {
					echo sprintf('(%s%%) oldPrice: %s newPrice: %s %s', $percent, $product->price, $product->purchase + ($product->purchase * ($percent / 100)), $product->name) . PHP_EOL;
				}
			}
		}
	}
